...
function editorCtrlS() {
  ...
  editor3.save();
  var content3 = editor3.getValue();

  $.ajax({
      url: "./content_block_edit_up.php",
      type: "POST",
      data:{
        'name':name,
        'file_html':content1,
...
<style>
  ...
  #wrapper {margin:0px; position: relative;}
  #container {padding: 0px;}
  .tbl_wrap {margin: 0px;} .tbl_frm01 td {padding:0px; background: transparent;}
...
<form name="fconfigform" id="fconfigform" method="post" action="./content_block_edit_up.php" enctype="MULTIPART/FORM-DATA">
...
<div id="anc_cf_block">
  ...
  <div id="right">
    ...
    <div class='area2'>
      ...
      <script>
      var textarea = document.getElementById('file_js');
      var editor3 = CodeMirror.fromTextArea(textarea, {
          lineNumbers: true,
          lineWrapping: true,
          styleActiveLine: true,
          scrollbarStyle: "simple",
      ...
      <div class="tbl_frm01 tbl_wrap">
        <table>
          <tbody>
                ...
                <?php
                    $admin_logo_img = G5_DATA_PATH."/common/admin_logo_img";
                    if (file_exists($admin_logo_img)){
                        $size = getimagesize($admin_logo_img);
                    ?>
                    <input type="checkbox" name="admin_logo_img_del" value="1" id="admin_logo_img_del">
                    <label for="admin_logo_img_del"><span class="sound_only">관리자 상단로고이미지</span> 삭제</label>
                    ...
                    <?php
                    // 폴더명 지정
                    $dir = G5_THEME_PATH."/template/".$name."/images";
                    $srcURL = G5_THEME_URL."/template/".$name."/images";
                    $srcURL2 = "<?=G5_THEME_URL?>/template/".$name."/images";
                    //echo $srcURL;
                            ...
                            if($dh = opendir($dir)) {
                                while(($entry = readdir($dh)) !== false) {
                                    ...
                                    if(is_dir($subdir)) {
                                        ...
                                        echo '
                                        <tr>
                                        <th scope="row" style="text-align:center;"><img src="'.$srcURL.'/'.$entry.'" style="width:80px !important"></th>
...
<div class="btn_fixed_top btn_confirm">
  <a href="./content_block.php" class="btn btn_02">취소</a>
