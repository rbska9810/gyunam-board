...
<div class="vst-write-wrap">
    ...
    <div class="vst-form-group">
        <label for="wr_subject" class="vst-label"><i class="fa fa-circle"></i> 매장타입</label>
        <select name="wr_6" id="wr_6" class="vst-select">
            <option value="기존" <?php echo ($write['wr_6'] == "기존") ? "selected" : ""; ?>>기존</option>
            <option value="신규" <?php echo ($write['wr_6'] == "신규") ? "selected" : ""; ?>>신규</option>
        </select>
    </div>

    <div class="vst-form-group">
        <label for="wr_subject" class="vst-label"><i class="fa fa-circle"></i> 매장명</label>
        <input type="text" name="wr_subject" value="<?php echo $subject ?>" id="wr_subject" required class="vst-input" placeholder="매장명을 입력해주세요.">
    </div>

    <div class="vst-form-group">
        <label for="wr_2" class="vst-label"><i class="fa fa-circle"></i> 전화번호</label>
        <input type="text" name="wr_2" value="<?php echo $wr_2 ?>" id="wr_2" required class="vst-input" placeholder="전화번호를 입력해주세요.">
    </div>

    <div class="vst-form-group">
        <label for="wr_4" class="vst-label"><i class="fa fa-circle"></i> 주소</label>
        <input type="text" name="wr_4" value="<?php echo $wr_4 ?>" id="wr_4" required class="vst-input" placeholder="주소를 입력해주세요. (자동으로 위도/경도가 변환됩니다)">
    </div>

    <div class="vst-form-group">
        <label for="wr_5" class="vst-label"><i class="fa fa-circle"></i> 네이버지도링크</label>
        <input type="text" name="wr_5" value="<?php echo $wr_5 ?>" id="wr_5" required class="vst-input" placeholder="네이버지도링크를 입력해주세요.">
    </div>

    <div class="vst-form-group">
        <label for="wr_3" class="vst-label"><i class="fa fa-circle"></i> 영업시간</label>
        <input type="text" name="wr_3" value="<?php echo $wr_3 ?>" id="wr_3" required class="vst-input" placeholder="영업시간을 입력해주세요.">
    </div>
...
var is_geocoding_success = false;                    

$("#fwrite").on("submit", function(e) {

    if (is_geocoding_success) {
        return true;
    }

    e.preventDefault();
    console.log("----- 폼 제출 일시 정지. 로직 시작 -----");
    ...
    console.log("주소 변환 시작: " + addr_input);

    if (typeof kakao === 'undefined' || !kakao.maps || !kakao.maps.services) {
        alert("카카오맵 API가 로드되지 않았습니다. 관리자에게 문의하세요.");
        // API 오류여도 글쓰기는 진행되어야 한다면 return true; 로 변경
        return false; 
    }
    ...
    geocoder.addressSearch(addr_input, function(result, status) {
        if (status === kakao.maps.services.Status.OK) {

            console.log("변환 성공! 위도:" + result[0].y + ", 경도:" + result[0].x);

            if($("#wr_7").length > 0) $("#wr_7").val(result[0].y);
            if($("#wr_8").length > 0) $("#wr_8").val(result[0].x);

            is_geocoding_success = true;

            f.submit(); // 변환 성공 후 폼 제출 재개

        } else {
            console.error("변환 실패 상태코드: " + status);
            alert("입력하신 주소를 지도에서 찾을 수 없습니다. 정확한 도로명 주소를 입력해주세요.");
